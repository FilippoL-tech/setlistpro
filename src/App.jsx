...
function buildPrintHTML(setlist, useItalian=false) {
  ...
  const rows = setlist.songs.map((s,i)=>{
    const tags = (s.tags||[]).map(t=>{
      const tp=TAG_PRESETS.find(x=>x.id===t);
  ...
  <div class="meta">
    ...
    <span>⏱ ${totalStr} totali</span>
    <span>🎵 ${setlist.songs.length} brani</span>
  </div>
  <table>
    <thead><tr>
...
function buildShareURL(setlist) {
  ...
  return `${window.location.origin}${window.location.pathname}?setlist=${b64}`;
}

function parseSharedSetlist() {
  try {
    const raw = new URLSearchParams(window.location.search).get("setlist");
    if (!raw) return null;
...
const defaultSetlist = () => ({
  id: uid(), name: "Il mio primo live",
  date: new Date().toISOString().slice(0,10), venue: "",
  songs: [
...
const defaultLibSong = () => ({
  id:uid(), title:"", key:"C", mode:"Maj", bpm:120, duration:"3:00", notes:"", tags:[], pdfName:null, pdfData:null
});
...
const IcMetro    = ({size=18}) => <Ic size={size} d="M12 2L8 8H4l4 4-2 8 6-4 6 4-2-8 4-4h-4L12 2z"/>;
...
function PDFViewer({ song, onClose }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const b64=song.pdfData.split(",")[1], bin=atob(b64);
  ...
  useEffect(() => {
    ...
    const render=async()=>{
      ...
      const scale=(canvas.parentElement?.clientWidth||600)/vp.width;
      const scaled=p.getViewport({scale});
      canvas.width=scaled.width; canvas.height=scaled.height;
      const task=p.render({canvasContext:canvas.getContext("2d"),viewport:scaled});
      renderTaskRef.current=task; try{await task.promise;}catch{}
    };
    render();
  }, [pdfDoc, page]);
  return (
    <div className="pdf-overlay" onClick={onClose}>
      <div className="pdf-modal" onClick={e=>e.stopPropagation()}>
        <div className="pdf-modal-header">
          <div className="pdf-modal-title"><IcPDF size={15}/><span>{song.pdfName}</span></div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            {numPages>1&&<div className="pdf-nav">
              <button className="btn-icon" onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page<=1}><IcLeft/></button>
              <span className="pdf-page-info">{page}/{numPages}</span>
              <button className="btn-icon" onClick={()=>setPage(p=>Math.min(numPages,p+1))} disabled={page>=numPages}><IcRight/></button>
            </div>}
            <button className="btn-icon" onClick={onClose}><IcX size={20}/></button>
          </div>
        </div>
        <div className="pdf-canvas-wrap">
          {loading&&<div className="pdf-status"><div className="pdf-spinner"/><span>Caricamento…</span></div>}
          {error&&<div className="pdf-status pdf-error"><IcPDF size={32}/><span>{error}</span></div>}
...
// ── TAG PILL / PICKER ─────────────────────────────────────────────────────────
function TagPill({ tagId, onRemove }) {
  const t = TAG_PRESETS.find(x=>x.id===tagId);
  if (!t) return null;
  return (
    <span className="tag-pill" style={{"--tc": t.color}}>
      {t.label}
      {onRemove && <button onClick={()=>onRemove(tagId)} className="tag-remove">×</button>}
    </span>
  );
}
function TagPicker({ selected, onChange }) {
  return (
    <div className="tag-picker">
      {TAG_PRESETS.map(t => (
        <button key={t.id} className={`tag-option${selected.includes(t.id)?" selected":""}`}
          style={{"--tc": t.color}}
          onClick={() => onChange(selected.includes(t.id)?selected.filter(x=>x!==t.id):[...selected,t.id])}>
          {selected.includes(t.id)&&<IcCheck size={11}/>} {t.label}
        </button>
      ))}
    </div>
  );
...
// ── METRONOME ─────────────────────────────────────────────────────────────────
function Metronome({ bpm: initialBpm }) {
  ...
  const playClick=useCallback((isAccent)=>{
    ...
    gain.gain.setValueAtTime(isAccent?.7:.4,ctx.currentTime);
    gain.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+0.06);
    osc.start();osc.stop(ctx.currentTime+0.06);
  },[]);
  useEffect(()=>{
    if(!running){clearInterval(intervalRef.current);return;}
    let cb=0;const tick=()=>{playClick(cb===0);setBeat(cb);cb=(cb+1)%beatsPerBar;};
    tick();intervalRef.current=setInterval(tick,(60/bpm)*1000);
    return()=>clearInterval(intervalRef.current);
  },[running,bpm,beatsPerBar,playClick]);
  const tapTempo=()=>{
    const now=Date.now(),taps=tapTimesRef.current;taps.push(now);
    if(taps.length>8)taps.shift();
    if(taps.length>=2){const diffs=taps.slice(1).map((t,i)=>t-taps[i]);setBpm(Math.round(60000/(diffs.reduce((a,b)=>a+b,0)/diffs.length)));}
  };
  return (
    <div className="metronome">
      <div className="metro-header"><IcMetro size={16}/><span>Metronomo</span></div>
      <div className="metro-beats">{Array.from({length:beatsPerBar}).map((_,i)=>(
        <div key={i} className={`beat-dot${running&&beat===i?" beat-active":""}${i===0?" beat-accent":""}`}/>
      ))}</div>
      <div className="metro-bpm-row">
        <button className="metro-adj" onClick={()=>setBpm(b=>Math.max(20,b-1))}>−</button>
        <div className="metro-bpm">{bpm}<span>BPM</span></div>
        <button className="metro-adj" onClick={()=>setBpm(b=>Math.min(300,b+1))}>+</button>
      </div>
      <input type="range" min={20} max={300} value={bpm} onChange={e=>setBpm(+e.target.value)} className="metro-slider"/>
      <div className="metro-controls">
        <select className="metro-select" value={beatsPerBar} onChange={e=>setBeatsPerBar(+e.target.value)}>
          {[2,3,4,5,6,7,8].map(n=><option key={n} value={n}>{n}/4</option>)}
        </select>
        <button className="metro-tap" onClick={tapTempo}>TAP</button>
        <button className={`metro-play${running?" running":""}`} onClick={()=>setRunning(r=>!r)}>
          {running?<IcPause size={20}/>:<IcPlay size={20}/>}
        </button>
      </div>
    </div>
  );
...
function StageMode({ setlist, onClose, useItalian }) {
  ...
  const expected=parseDur(song?.duration);const pct=expected>0?Math.min(1,elapsed/expected):0;const over=expected>0&&elapsed>expected;
  return (
    <div className="stage-overlay">
      <button className="stage-close" onClick={onClose}><IcX size={24}/></button>
      <div className="stage-progress-bar"><div className="stage-progress-fill" style={{width:`${pct*100}%`,background:over?"#e8604a":"#e8c84a"}}/></div>
      ...
      <div className="stage-main">
        ...
        <div className={`stage-notes${song?.notes?"":" stage-notes-empty"}`}>
          {song?.notes?song.notes:<span className="stage-notes-placeholder">Nessuna nota per questo brano</span>}
        </div>
      </div>
      <div className={`stage-timer${over?" stage-timer-over":""}`}>
        {fmt(elapsed)}{expected>0&&<span className="stage-timer-expected"> / {fmt(expected)}</span>}
      </div>
      <div className="stage-controls">
        <button className="stage-btn" onClick={()=>goTo(Math.max(0,idx-1))} disabled={idx===0}><IcSkipL size={28}/></button>
        <button className="stage-btn stage-btn-main" onClick={()=>setRunning(r=>!r)}>{running?<IcPause size={36}/>:<IcPlay size={36}/>}</button>
        <button className="stage-btn" onClick={()=>{setElapsed(0);setRunning(false);}}><IcReset size={24}/></button>
        <button className="stage-btn" onClick={()=>goTo(Math.min(setlist.songs.length-1,idx+1))} disabled={idx===setlist.songs.length-1}><IcSkipR size={28}/></button>
      </div>
      <div className="stage-list">{setlist.songs.map((s,i)=>(
      ...
      {showMetro&&<div className="stage-metro-wrap"><Metronome bpm={song?.bpm}/></div>}
    </div>
  );
}
...
function HistoryModal({ setlist, onUpdate, onClose }) {
  ...
  const addEntry=()=>{
    if(!form.venue&&!form.notes)return;
    onUpdate({...setlist,history:[{id:uid(),...form},...history]});
    setForm({date:new Date().toISOString().slice(0,10),venue:"",notes:"",rating:5});
  };
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box history-modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="history-header"><IcHistory size={20}/><h2>Storico Live</h2></div>
        <p className="history-sub">Tieni traccia di ogni volta che hai suonato questa scaletta.</p>
        <div className="history-form">
          <div className="hf-row">
            <input type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))} className="hf-input"/>
            <input value={form.venue} placeholder="Venue / locale" onChange={e=>setForm(f=>({...f,venue:e.target.value}))} className="hf-input hf-grow"/>
          </div>
          <textarea value={form.notes} placeholder="Note post-live…" rows={2} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} className="hf-textarea"/>
          <div className="hf-row hf-bottom">
            <div className="rating-row">{[1,2,3,4,5].map(n=>(
              <button key={n} className={`star-btn${form.rating>=n?" active":""}`} onClick={()=>setForm(f=>({...f,rating:n}))}>
                <IcStar size={20} fill={form.rating>=n}/>
              </button>
            ))}</div>
            <button className="btn-add-history" onClick={addEntry}>+ Aggiungi</button>
          </div>
        </div>
        <div className="history-list">
          {history.length===0&&<p className="history-empty">Nessun live registrato ancora.</p>}
          {history.map(h=>(
            <div key={h.id} className="history-entry">
              <div className="he-top">
                <div className="he-info">
                  <span className="he-date">📅 {h.date}</span>
                  {h.venue&&<span className="he-venue">📍 {h.venue}</span>}
                </div>
                <div className="he-right">
                  <div className="he-stars">{[1,2,3,4,5].map(n=><span key={n} className={`he-star${h.rating>=n?" active":""}`}>★</span>)}</div>
                  <button className="btn-icon btn-delete" onClick={()=>onUpdate({...setlist,history:history.filter(x=>x.id!==h.id)})}><IcTrash/></button>
                </div>
              </div>
              {h.notes&&<p className="he-notes">{h.notes}</p>}
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="lib-overlay" onClick={onClose}>
      <div className="lib-panel" onClick={e=>e.stopPropagation()}>
        ...
        <div className="lib-header">
          <div className="lib-title"><IcBook size={20}/><h2>Libreria Brani</h2></div>
          <button className="btn-icon" onClick={onClose}><IcX size={20}/></button>
        </div>
        <div className="lib-toolbar">
          <div className="lib-search-wrap">
            <IcSearch size={16}/>
            <input className="lib-search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cerca brano…"/>
        ...
        {usedTags.length>0&&(
          <div className="lib-tag-filter">
            ...
            {usedTags.map(t=>{const tp=TAG_PRESETS.find(x=>x.id===t);return tp?(
              <button key={t} className={`tag-filter-btn${filterTag===t?" active":""}`} style={{"--tc":tp.color}} onClick={()=>setFilterTag(filterTag===t?null:t)}>{tp.label}</button>
            ):null;})}
          </div>
        )}
        <div className="lib-list">
          {filtered.length===0&&<div className="lib-empty"><IcMusic size={32}/><p>Nessun brano trovato.</p></div>}
          {filtered.map(song=>(
            <div key={song.id} className={`lib-song${editId===song.id?" lib-song-open":""}`}>
              <div className="lib-song-main">
                <div className="lib-song-info">
                  <span className="lib-song-title">{song.title||"Senza titolo"}</span>
                  <span className="lib-song-key">{song.key} {song.mode} · {song.bpm} BPM · {song.duration}</span>
                  {song.tags?.length>0&&<div className="lib-tags">{song.tags.map(t=><TagPill key={t} tagId={t}/>)}</div>}
                </div>
                <div className="lib-song-actions">
                  {onAddToSetlist&&<button className="btn-lib-add" onClick={()=>onAddToSetlist(song)}><IcImport size={15}/> Usa</button>}
                  {song.pdfName&&<button className="btn-icon btn-pdf has-pdf" onClick={()=>setViewingPDF(song)}><IcPDF/></button>}
                  <button className="btn-icon" onClick={()=>setEditId(editId===song.id?null:song.id)}><IcEdit/></button>
                  <button className="btn-icon btn-delete" onClick={()=>deleteSong(song.id)}><IcTrash/></button>
                </div>
              </div>
              {editId===song.id&&(
                <div className="lib-song-edit">
                  ...
                  <div className="lib-pdf-row">
                    {song.pdfName
                      ?<div className="pdf-badge"><IcPDF/><span>{song.pdfName}</span>
                          <button className="btn-text" onClick={()=>setViewingPDF(song)}>Visualizza</button>
                          <button className="btn-text red" onClick={()=>updateSong({...song,pdfName:null,pdfData:null})}>Rimuovi</button>
                        </div>
                      :<button className="btn-text" onClick={()=>fileRefs.current[song.id]?.click()}>+ Allega PDF</button>
                    }
                    <input ref={el=>fileRefs.current[song.id]=el} type="file" accept=".pdf" style={{display:"none"}}
                      onChange={e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>updateSong({...song,pdfName:file.name,pdfData:ev.target.result});r.readAsDataURL(file);e.target.value="";}}/>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="lib-footer">{library.length} brani in libreria</div>
      </div>
    </div>
  );
}
...
function SongRow({ song, index, onUpdate, onDelete, onOpenPDF, isPro, onShowProBanner, useItalian,
  ...
  return (
    <div className={`song-row${dragging?" dragging":""}`}
      draggable onDragStart={e=>onDragStart(e,index)} onDragOver={e=>onDragOver(e,index)}
      onDrop={e=>onDrop(e,index)} onDragEnd={onDragEnd}>
      <div className="song-main">
        <span className="grip"><IcGrip/></span>
        <span className="song-num">{index+1}</span>
        <div className="song-title-col">
          <input className="song-title" value={song.title} onChange={e=>onUpdate({...song,title:e.target.value})} placeholder="Titolo brano"/>
        ...
        <input ref={fileRef} type="file" accept=".pdf" style={{display:"none"}}
          onChange={e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>onUpdate({...song,pdfName:file.name,pdfData:ev.target.result});r.readAsDataURL(file);e.target.value="";}}/>
      </div>
      {expanded&&(
        <div className="song-notes">
          ...
          {song.pdfName&&(
            <div className="pdf-badge">
              ...
              <button className="btn-text" onClick={()=>onOpenPDF(song)}>📄 Visualizza</button>
              <button className="btn-text red" onClick={()=>onUpdate({...song,pdfName:null,pdfData:null})}>Rimuovi</button>
            </div>
          )}
        </div>
      )}
...
function PrintModal({ setlist, onClose, useItalian=false }) {
  ...
  const doPrint=()=>{
    ...
    const a=document.createElement("a");
    a.href=url; a.download=`${setlist.name.replace(/[^a-z0-9]/gi,"_")}_scaletta.html`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    setTimeout(()=>URL.revokeObjectURL(url),5000);
  };
  return (
    <div className="modal-overlay print-modal-overlay" onClick={onClose}>
      <div className="modal-box print-modal-box" onClick={e=>e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}><IcX/></button>
        <div id="print-content">
          <div className="pm-header">
            <h1 className="pm-title">{setlist.name}</h1>
          ...
          <div className="pm-footer">Generato con SetlistPro · {new Date().toLocaleDateString("it-IT")}</div>
        </div>
        <div className="pm-actions no-print">
          <button className="btn-cancel" onClick={onClose}>Chiudi</button>
          <button className="btn-upgrade" onClick={doPrint}><IcPrint size={16}/> Scarica HTML → Apri → Stampa</button>
        </div>
      </div>
    </div>
  );
...
function TransposeModal({ setlist, onUpdate, onClose }) {
  ...
  const apply=()=>{onUpdate({...setlist,songs:preview});onClose();};
  const stepLabel=n=>n===0?"Originale":`${n>0?"+":""}${n} semitoni`;
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box transpose-modal" onClick={e=>e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}><IcX/></button>
        <div className="transpose-header"><IcTranspose size={22}/><h2>Trasposizione</h2></div>
        <p className="transpose-sub">Trasporta tutte le tonalità della scaletta in un colpo solo.</p>
        <div className="transpose-control">
          <button className="metro-adj" onClick={()=>setSemitones(s=>Math.max(-11,s-1))}>−</button>
          <div className="transpose-value"><span className="transpose-num">{stepLabel(semitones)}</span></div>
          <button className="metro-adj" onClick={()=>setSemitones(s=>Math.min(11,s+1))}>+</button>
        </div>
        <input type="range" min={-11} max={11} value={semitones} onChange={e=>setSemitones(+e.target.value)} className="metro-slider"/>
        <div className="transpose-preview">{setlist.songs.map((s,i)=>(
          <div key={s.id} className="tp-row">
            <span className="tp-title">{s.title}</span>
            <span className="tp-keys">
              <span className="tp-old">{s.key} {s.mode}</span>
              {semitones!==0&&<><span className="tp-arrow">→</span><span className="tp-new">{preview[i].key} {s.mode}</span></>}
            </span>
          </div>
        ))}</div>
        <div className="transpose-actions">
          <button className="btn-cancel" onClick={onClose}>Annulla</button>
          <button className="btn-upgrade" onClick={apply} disabled={semitones===0}>Applica trasposizione</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box share-modal" onClick={e=>e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}><IcX/></button>
        <div className="share-header"><IcShare2 size={22}/><h2>Condividi Scaletta</h2></div>
        <p className="share-sub">Chiunque abbia questo link può visualizzare la scaletta in sola lettura. Gli spartiti PDF non sono inclusi.</p>
        <div className="share-url-box"><span className="share-url-text">{url.length>80?url.slice(0,80)+"…":url}</span></div>
        <button className={`btn-copy${copied?" copied":""}`} onClick={copy}>
          {copied?<><IcCheck size={16}/> Copiato!</>:<><IcShare2 size={16}/> Copia link</>}
        </button>
        <div className="share-info">
          <div className="share-info-row"><span>🎵</span><span>{setlist.songs.length} brani</span></div>
          <div className="share-info-row"><span>📅</span><span>{setlist.date}</span></div>
          {setlist.venue&&<div className="share-info-row"><span>📍</span><span>{setlist.venue}</span></div>}
        </div>
        <p className="modal-note">Il link contiene tutti i dati della scaletta codificati — nessun server necessario.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="app">
      <header className="app-header">
        ...
        <span className="shared-badge">👁 Sola lettura</span>
      </header>
        ...
        {setlist.songs.map((s,i)=>(
          <div key={s.id} className="song-row">
            <div className="song-main">
              ...
              <div className="song-title-col">
                <span className="song-title" style={{cursor:"default"}}>{s.title}</span>
                {s.tags?.length>0&&<div className="song-inline-tags">{s.tags.map(t=><TagPill key={t} tagId={t}/>)}</div>}
              </div>
              <div className="song-meta" style={{pointerEvents:"none"}}>
                <span className="shared-key">{s.key} {s.mode}</span>
                <span className="shared-bpm">{s.bpm} BPM</span>
                <span className="shared-dur">{s.duration}</span>
              </div>
            </div>
            {s.notes&&<div className="song-notes" style={{paddingTop:8}}><p style={{fontSize:".82rem",color:"var(--muted)"}}>{s.notes}</p></div>}
          </div>
        ))}
      </div>
      <div className="shared-footer">
        <p>Vuoi creare le tue scalette?</p>
        <button className="btn-upgrade" style={{marginTop:8}} onClick={onDismiss}>Prova SetlistPro gratis</button>
      </div>
    </div>
  );
}
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e=>e.stopPropagation()}>
        ...
        <ul className="pro-features">
          ...
          <li>✓ PDF spartiti allegabili</li>
          <li>✓ Modalità palco completa</li>
          <li>✓ Metronomo, Libreria, Storico live</li>
          <li>✓ Trasposizione e condivisione</li>
...
function SetlistEditor({ setlist, onUpdate, onBack, library, onUpdateLibrary, isPro, useItalian, onToggleItalian }) {
  ...
  const addSong=()=>{
    if(!isPro&&setlist.songs.length>=FREE_SONGS_LIMIT){setLimitWarning(true);return;}
    const s={id:uid(),title:"Nuovo brano",key:"C",mode:"Maj",bpm:120,duration:"3:00",notes:"",tags:[],pdfName:null,pdfData:null};
    onUpdate({...setlist,songs:[...setlist.songs,s]});
  };
  const addFromLibrary=(libSong)=>{
    if(!isPro&&setlist.songs.length>=FREE_SONGS_LIMIT){setLimitWarning(true);return;}
    onUpdate({...setlist,songs:[...setlist.songs,{...libSong,id:uid()}]});
  };
  ...
  const handleDrop=(e,i)=>{
    ...
    const songs=[...setlist.songs];const[m]=songs.splice(dragIdx,1);songs.splice(i,0,m);
    onUpdate({...setlist,songs});setDragIdx(null);setOverIdx(null);
  };
        ...
        <div className="editor-header-top">
          ...
          <div className="editor-header-actions">
            ...
            <button className="btn-icon-label" onClick={()=>setShowHistory(true)}>
              ...
              {(setlist.history?.length||0)>0&&<span className="history-badge">{setlist.history.length}</span>}
            </button>
            <button className="btn-icon-label" onClick={()=>setShowTranspose(true)}><IcTranspose size={15}/> Trasponi</button>
            <button className="btn-icon-label" onClick={()=>setShowShare(true)}><IcShare2 size={15}/> Condividi</button>
            <button className="btn-icon-label" onClick={()=>setShowPrint(true)}><IcPrint size={15}/> Stampa</button>
            <button className={`btn-icon-label${showMetro?" active":""}`} onClick={()=>setShowMetro(v=>!v)}><IcMetro size={15}/> Metro</button>
            <button className="btn-stage" onClick={()=>setStageMode(true)}><IcStage size={15}/> Palco</button>
          </div>
        </div>
        <input className="setlist-name-input" value={setlist.name} onChange={e=>onUpdate({...setlist,name:e.target.value})}/>
        <div className="editor-meta-row">
          <label>📅 <input type="date" value={setlist.date} onChange={e=>onUpdate({...setlist,date:e.target.value})}/></label>
          <label>📍 <input value={setlist.venue} placeholder="Venue / locale" onChange={e=>onUpdate({...setlist,venue:e.target.value})}/></label>
          <span className="total-dur"><IcClock/> {totalStr} · {setlist.songs.length} brani</span>
        </div>
      </div>

      {showMetro&&<div className="metro-panel"><Metronome bpm={setlist.songs[0]?.bpm||120}/></div>}
      ...
      <div className="songs-list">
        {setlist.songs.map((song,i)=>(
          <SongRow key={song.id} song={song} index={i} isPro={isPro} useItalian={useItalian}
            ...
            onOpenPDF={s=>setViewingPDF(s)}
            onDragStart={handleDragStart} onDragOver={handleDragOver}
            onDrop={e=>handleDrop(e,i)} onDragEnd={()=>{setDragIdx(null);setOverIdx(null);}}/>
        ))}
      </div>
      <div className="add-song-row">
        <button className="btn-add-song" onClick={addSong}><IcPlus/> Nuovo brano</button>
        <button className="btn-add-from-lib" onClick={()=>setShowLib(true)}><IcBook size={15}/> Da libreria</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [isPro]=useState(false); // diventerà true dopo acquisto Stripe
            ...
            <>
              <header className="app-header">
                ...
                <div className="header-right">
                  ...
                  {!isPro&&<button className="btn-pro-badge" onClick={()=>setShowUpgrade(true)}>
                    <IcStar size={13}/> Free {setlists.length}/{FREE_LIMIT}
                  </button>}
                </div>
              </header>
              ...
              <div className="setlists-grid">
                {setlists.map(sl=>(
                  <div key={sl.id} className="setlist-card" onClick={()=>setActiveId(sl.id)}>
                    <div className="card-top">
                      <IcList/>
                      <button className="btn-icon btn-delete card-del" onClick={e=>{e.stopPropagation();deleteSetlist(sl.id);}}><IcTrash/></button>
                    </div>
                    <h3>{sl.name}</h3>
                    ...
                    <div className="card-footer">
                      <span>{sl.songs.length} brani</span>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        {sl.history?.length>0&&<span className="card-history-badge"><IcHistory size={11}/> {sl.history.length}</span>}
                        <span className="card-arrow"><IcChevR/></span>
                      </div>
                    </div>
                  </div>
                ))}
                <button className="setlist-card card-new" onClick={createSetlist}>
                  <IcPlus size={32}/><span>Nuova scaletta</span>
                </button>
              </div>
            </>
          )}
...
.app-header{display:flex;align-items:center;justify-content:space-between;padding:20px 0 12px}
...
.stage-song-title{font-family:'Playfair Display',serif;font-size:clamp(2.8rem,10vw,5.5rem);font-weight:900;line-height:1;color:#fff;margin-bottom:20px;word-break:break-word}
...
.stage-notes{font-size:clamp(1rem,3vw,1.3rem);color:rgba(255,255,255,.75);background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:14px 22px;max-width:500px;width:100%;line-height:1.6;white-space:pre-wrap;text-align:left;margin-top:4px}
...
@media(max-width:500px){
  ...
  .stage-song-title{font-size:clamp(2rem,12vw,4rem)}
