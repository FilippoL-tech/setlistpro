...
// ── EXPORT / IMPORT (solo lista brani, gli allegati restano sul dispositivo) ──
const stripSong = (s) => ({
  title:s.title||"", key:s.key||"C", mode:s.mode||"Maj",
  bpm:typeof s.bpm==="number"?s.bpm:120, duration:s.duration||"",
  notes:s.notes||"", tags:Array.isArray(s.tags)?s.tags:[],
  pdfName:s.pdfName||null, mp3Name:s.mp3Name||null,
});
const rehydrateSong = (s) => ({
  id:uid(), title:(s&&s.title)||"Senza titolo", key:(s&&s.key)||"C", mode:(s&&s.mode)||"Maj",
  bpm:(s&&typeof s.bpm==="number")?s.bpm:120, duration:(s&&s.duration)||"",
  notes:(s&&s.notes)||"", tags:(s&&Array.isArray(s.tags))?s.tags:[],
  pdfName:null, pdfData:null, mp3Name:null, mp3Data:null,
});
const slugify = (s) => ((s||"scaletta").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"").slice(0,40))||"scaletta";
...
function exportSetlist(setlist){
  downloadJSON(`setlistpro-${slugify(setlist.name)}.json`,{
    app:"setlistpro", type:"setlist", version:1, exportedAt:new Date().toISOString(),
    setlist:{ name:setlist.name||"Scaletta", date:setlist.date||"", venue:setlist.venue||"", songs:(setlist.songs||[]).map(stripSong) }
  });
}
function exportLibrary(library){
  downloadJSON("setlistpro-libreria.json",{
    app:"setlistpro", type:"library", version:1, exportedAt:new Date().toISOString(),
    songs:(library||[]).map(stripSong)
  });
}
function readImportFile(file){
  return new Promise((resolve,reject)=>{
    const r=new FileReader();
    r.onload=()=>{ try{ const o=JSON.parse(r.result); if(o&&o.app==="setlistpro") resolve(o); else reject(new Error("Non sembra un file SetlistPro.")); }catch{ reject(new Error("File non valido o danneggiato.")); } };
...
const IcDownload=({size=18})=><Ic size={size} d="M12 3v12M8 11l4 4 4-4M5 21h14"/>;
const IcUpload  =({size=18})=><Ic size={size} d="M12 21V9M8 13l4-4 4 4M5 3h14"/>;
...
function Library({library,onUpdate,onClose,onAddToSetlist,isPro,useItalian}){
  ...
  const ioRef=useRef(); const [ioMsg,setIoMsg]=useState(null);
  const doImportLib=async(file)=>{
    try{
      const o=await readImportFile(file);
      const incoming=o.type==="library"?o.songs:((o.setlist&&o.setlist.songs)||o.songs||[]);
      if(!Array.isArray(incoming)||incoming.length===0){setIoMsg({err:true,t:"Nessun brano nel file."});return;}
      let toAdd=incoming.map(rehydrateSong); let note="";
      if(!isPro){
        const room=Math.max(0,FREE_LIB_LIMIT-library.length);
        if(toAdd.length>room) note=` — piano free: aggiunti ${room} (max ${FREE_LIB_LIMIT}), passa a PRO per tutti`;
        toAdd=toAdd.slice(0,room);
      }
      if(toAdd.length===0){setIoMsg({err:true,t:`Libreria piena (max ${FREE_LIB_LIMIT} nel piano free).`});return;}
      onUpdate([...library,...toAdd]);
      setIoMsg({err:false,t:`✓ Importati ${toAdd.length} brani${note}`});
...
const handleLogout = async () => {
  ...
  const importRef=useRef(); const [importMsg,setImportMsg]=useState(null);
  const importSetlistFromFile=async(file)=>{
    try{
      const o=await readImportFile(file);
      const sl=o.setlist||(o.type==="library"?{name:"Brani importati",songs:o.songs}:null);
      if(!sl||!Array.isArray(sl.songs)||sl.songs.length===0){setImportMsg({err:true,t:"Nessuna scaletta valida nel file."});return;}
      if(!isPro&&setlists.length>=FREE_LIMIT){setShowUpgrade(true);return;}
      let songs=sl.songs.map(rehydrateSong); let note="";
      if(!isPro&&songs.length>FREE_SONGS_LIMIT){note=` (piano free: importati i primi ${FREE_SONGS_LIMIT} brani)`;songs=songs.slice(0,FREE_SONGS_LIMIT);}
      const ns={...defaultSetlist(),id:uid(),name:sl.name||"Scaletta importata",date:sl.date||new Date().toISOString().slice(0,10),venue:sl.venue||"",songs,history:[]};
      setData(d=>({...d,setlists:[...d.setlists,ns]}));setActiveId(ns.id);
      if(note)setImportMsg({err:false,t:`Scaletta importata${note}`});
    }catch(e){setImportMsg({err:true,t:e.message||"Import fallito."});}
  };
          ...
          {active?(
            ...
            <>
              ...
              <div className="setlists-grid">
                ...
                <button className="setlist-card card-new" onClick={()=>importRef.current&&importRef.current.click()}><IcUpload size={28}/><span>Importa scaletta</span></button>
                <input ref={importRef} type="file" accept=".json,application/json" style={{display:"none"}} onChange={e=>{const f=e.target.files[0];if(f)importSetlistFromFile(f);e.target.value="";}}/>
              </div>
              {importMsg&&<div style={{margin:"4px 2px 0",fontSize:".85rem",color:importMsg.err?"#e8604a":"var(--accent)"}}>{importMsg.t}</div>}
