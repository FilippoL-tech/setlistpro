...
function PDFViewer({song,onClose}){
  ...
  useEffect(()=>{
    ...
    const render=async()=>{if(renderTaskRef.current){try{renderTaskRef.current.cancel();}catch{}}const p=await pdfDoc.getPage(page);const vp=p.getViewport({scale:1});const scale=(canvas.parentElement?.clientWidth||600)/vp.width;const scaled=p.getViewport({scale});canvas.width=scaled.width;canvas.height=scaled.height;const task=p.render({canvasContext:canvas.getContext("2d"),viewport:scaled});renderTaskRef.current=task;try{await task.promise;}catch{}};
    render();
  },[pdfDoc,page]);
...
function Metronome({bpm:initialBpm}){
  const [bpm,setBpm]=useState(initialBpm||120),[running,setRunning]=useState(false),[beat,setBeat]=useState(0),[beatsPerBar,setBeatsPerBar]=useState(4);
...
function Library({library,onUpdate,onClose,onAddToSetlist,isPro}){
  ...
  const addSong=()=>{if(!isPro&&library.length>=FREE_SONGS_LIMIT){setLibWarning(true);return;}const s={...defaultLibSong(),title:"Nuovo brano"};onUpdate([...library,s]);setEditId(s.id);};
