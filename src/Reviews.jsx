...
/* Stella SVG con riempimento parziale */
...
export default function ReviewsButton({ user }) {
  ...
  // precompila il form quando apro il modale, se ho già recensito
  const openModal = () => {
    if (mine) { setRating(mine.rating); setText(mine.body); setBand(mine.author_name); }
    setErr(null);
    setOpen(true);
  };

  const submit = async () => {
    if (!user) { setErr("Devi essere loggato per recensire."); return; }
    if (!rating || !text.trim()) return;
    setBusy(true); setErr(null);
    const name = band.trim() || (user.email ? user.email.split("@")[0] : "Musicista");
    const { error } = await supabase.from("reviews").upsert(
      { user_id: user.id, author_name: name, rating, body: text.trim() },
      { onConflict: "user_id" }
    );
    setBusy(false);
      ...
      <div className="rv-cta">
        <div className="rv-cta-left">
          ...
          {loading ? <p className="rv-cta-sub">…</p> : count ? (
            <div className="rv-cta-rating">
              <span className="rv-avg">{avg.toFixed(1)}</span>
              <StarRow value={avg} size={20} />
        ...
        <button className="rv-btn" onClick={openModal}>★ {mine ? "Modifica la tua recensione" : "Lascia una recensione"}</button>
      </div>
        ...
        <div className="rv-overlay" onClick={() => setOpen(false)}>
          <div className="rv-modal" onClick={e => e.stopPropagation()}>
            <div className="rv-modal-head">
              <h3>Recensioni</h3>
              <button className="rv-x" onClick={() => setOpen(false)}>✕</button>
            </div>
                    ...
                    <div key={r.id} className="rv-item">
                      ...
                      <div className="rv-item-body">
                        <div className="rv-item-head">
                          <span className="rv-item-name">{r.author_name}{mine && r.id === mine.id && <span className="rv-you"> · la tua</span>}</span>
...
@media (max-width:480px){.rv-cta{flex-direction:column;align-items:flex-start;}.rv-btn{width:100%;}}
`;
